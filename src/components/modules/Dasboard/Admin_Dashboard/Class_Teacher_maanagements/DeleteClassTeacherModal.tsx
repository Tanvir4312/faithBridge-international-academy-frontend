...
import { deleteClassTeacher } from "@/services/admin-srever-action/class-teacher.service"
import { toast } from "sonner"
import { Badge } from "@/components/ui/badge"
import Image from "next/image"

interface DeleteClassTeacherModalProps {
    classTeacher: IClassTeacher | null
    isOpen: boolean
    onOpenChange: (open: boolean) => void
}

const DeleteClassTeacherModal = ({ classTeacher, isOpen, onOpenChange }: DeleteClassTeacherModalProps) => {

    const queryClient = useQueryClient()
    const [isConfirming, setIsConfirming] = useState(false)

    const mutation = useMutation({
        mutationFn: () => deleteClassTeacher(classTeacher!.id),
        onSuccess: (data: any) => {
            if(data?.success) {
                queryClient.invalidateQueries({ queryKey: ["class-teachers"] })
                toast.success("Assignment Terminated", {
                    description: `${classTeacher?.teacher.name} is no longer assigned to Class ${classTeacher?.class.name}.`,
            ...
            <Dialog open={isOpen} onOpenChange={(open) => {
                ...
                >
                    <DialogHeader className="sr-only">
                        <DialogTitle>Class Teacher Assignment Removal</DialogTitle>
                        <DialogDescription>Review assignment details before removal for {classTeacher.teacher.name}</DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="relative bg-rose-600 px-6 py-5 md:px-8 md:py-6 flex items-center justify-between overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 md:gap-4 text-white relative z-10">
                            ...
                            <h2 className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em]">Assignment Deletion Review</h2>
                        </div>
                        <DialogClose asChild>
                    ...
                    <div className="p-6 md:p-10 space-y-8 md:space-y-10 text-center max-h-[80vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="space-y-6 flex flex-col items-center">
                            <div className="h-24 w-24 md:h-28 md:w-28 rounded-full border-4 border-rose-100 p-1 relative group">
                                <div className="h-full w-full rounded-full overflow-hidden relative bg-muted flex items-center justify-center shadow-inner">
                                    {classTeacher.teacher.profilePhoto ? (
                                        <Image
                                            src={classTeacher.teacher.profilePhoto}
                                            alt={classTeacher.teacher.name}
                                            fill
                            ...
                            <div className="space-y-2">
                                <Badge variant="outline" className="border-rose-200 text-rose-600 font-black uppercase text-[8px] md:text-[9px] tracking-widest px-4 py-0.5 rounded-full">
                                    Class Assignment Marked for Deletion
                                </Badge>
                                ...
                                <div className="flex items-center justify-center gap-2 mt-2">
                                    ...
                                    <p className="text-[10px] md:text-[11px] font-bold text-muted-foreground uppercase tracking-widest">
                                        Class: <span className="text-emerald-600">{classTeacher.class.name}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-5 md:p-6 rounded-[1.5rem] md:rounded-[2rem] bg-amber-50/80 border border-amber-100 flex items-start gap-4 text-left">
                            ...
                            <div className="space-y-1">
                                <p className="text-[10px] md:text-[11px] font-black uppercase text-amber-900 leading-none">Warning</p>
                                <p className="text-[11px] md:text-xs font-medium text-amber-800/70 leading-relaxed">
                                    Removing this assignment will leave Class <span className="font-bold">{classTeacher.class.name}</span> without a designated class teacher. You will need to assign a new teacher manually.
                                </p>
                            </div>
                        </div>
            ...
            <Dialog open={isConfirming} onOpenChange={setIsConfirming}>
                ...
                >
                    <DialogHeader className="sr-only">
                        ...
                        <DialogDescription>Final check before removing the class teacher assignment</DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="p-8 md:p-10 space-y-8 text-center">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-xs md:text-sm font-medium text-muted-foreground leading-relaxed px-2">
                                This will permanently remove the assignment record. This action cannot be undone.
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            >
                                {mutation.isPending ? "Removing..." : "Yes, Delete Assignment"}
                            </Button>
                            <Button
                                variant="ghost"
                                onClick={() => {
...
export default DeleteClassTeacherModal
