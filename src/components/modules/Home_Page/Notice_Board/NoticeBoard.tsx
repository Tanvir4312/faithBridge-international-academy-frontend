...
const NoticeBoard = () => {
 ...
 return (
  <div className='pt-5 md:pt-0'>
   <p className='text-3xl font-bold text-[#007B5E] dark:text-green-400 transition-colors duration-300 pl-4'>Notice Board</p>
     ...
     notice.type === "GENERAL" && (
      ...
      >
       <div className=' h-12 border-b md:px-0 px-2 flex items-center'>
        <p className='mr-3 bg-[#007B5E] dark:bg-green-600 transition-colors duration-300 px-2 py-2 rounded text-white mb-3'>{new Date(notice?.createdAt).toLocaleDateString()}</p>
        <p>
         <span className='block'>{notice.title}</span>
         <Link href={`/notices/notice/${notice?.id}`}> <span className='pb-2 font-medium text-[#007B5E] dark:text-orange-400 cursor-pointer transition-colors duration-300'>View Notice</span></Link>
        </p>
       </div>
